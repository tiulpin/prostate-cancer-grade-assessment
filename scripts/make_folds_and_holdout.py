...
def main():
    ...
    train_data, holdout_data = train_test_split(
        data, test_size=0.15, random_state=SEED, shuffle=True, stratify=data.isup_grade
    )
    ...
    for fold, (trn_idx, val_idx) in enumerate(
        ...
        train_data.loc[trn_idx].to_csv(
            f"{ROOT_PATH}/train_cleaned_{fold}.csv", index=False
        )
        train_data.loc[val_idx].to_csv(
            f"{ROOT_PATH}/val_cleaned_{fold}.csv", index=False
        )
